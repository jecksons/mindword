...
class ExpressionController {
   ...
   static async saveFromDraft(data, idUser, conn) {
      let transStarted = false;
      try {
         if (!data.id_draft) {
            throw new ErBadRequest('id_draft is missing!');
         }
         await conn.beginTransaction();
         transStarted = true;

         const draft = await DraftController.getByIdNT(data.id_draft, idUser, conn);
         const ret = await ExpressionController.saveNt({ description: draft.description }, idUser, conn);
         await DraftController.removeNT(data.id_draft, idUser, conn);
         await conn.commit();
         return ret;
      }
      catch (err) {
   ...
   static async getDayTask(idUser, day, conn) {
      try {
         ...
         const drafts = await DraftController.getAll(idUser, conn, true);
         return {
            score,
            items,
            annotations,
   ...
   static saveFromDraftReq(req, res, conn) {
      ExpressionController.saveFromDraft(req.body, 1, conn)
         .then((ret) => res.status(200).json(ret))
         .catch((err) => UtilsLib.resError(err, res));
   }
